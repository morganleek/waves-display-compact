...
const ScaleButtons = ( { zoom, setZoom } ) => (
	<div className="scaleButtons">
		{ zoom > 0 && (
			<>
				<button
					className={ classNames( { disabled: zoom >= 7 } ) }
					onClick={ () => {
						if ( zoom < 7 ) {
							setZoom( zoom + 1 );
						}
				...
				<button
					className={ classNames( { disabled: zoom <= 1 } ) }
					onClick={ () => {
						if ( zoom > 1 ) {
							setZoom( zoom - 1 );
						}
...
const BackToTop = () => (
	<button
		...
		onClick={ () => {
			document
				.querySelector( '.latest-observations' )
...
function App() {
	...
	arrowColours.forEach( ( colour ) => {
		...
		image.src =
			'data:image/svg+xml,' +
			encodeURIComponent( generateArrow( colour ) );
	...
	const processChartData = () => {
		...
		if ( buoyDataPoints.length > 0 ) {
			if ( buoyDataPoints[ 0 ]?.data_points ) {
				const unprocessedData = JSON.parse(
					buoyDataPoints[ 0 ]?.data_points
				);
				// console.log( unprocessedData );
				processedData.timeStampUTC =
					unprocessedData[ 'Timestamp (UTC)' ];
				processedData.surfaceTemperature =
					unprocessedData[ 'SST (degC)' ] !== '-9999.0'
						? parseFloat( unprocessedData[ 'SST (degC)' ] )
						: null;
				// processedData.swellHeight = unprocessedData['Hsig_swell (m)'] != "-9999.00" ? parseFloat( unprocessedData['Hsig_swell (m)'] ) : null;
				processedData.swellHeight =
					unprocessedData[ 'Hsig (m)' ] !== '-9999.00'
						? parseFloat( unprocessedData[ 'Hsig (m)' ] )
						: null;
				processedData.seasHeight =
					unprocessedData[ 'Hsig_sea (m)' ] !== '-9999.00'
						? parseFloat( unprocessedData[ ':' ] )
						: null;
				processedData.swellDirection =
					unprocessedData[ 'Dm (deg)' ] !== '-9999.00'
						? getSimpleDirection( unprocessedData[ 'Dm (deg)' ] )
						: null;
				processedData.windDirectionNumerical =
					unprocessedData[ 'WindDirec (deg)' ] !== '-9999.00'
						? unprocessedData[ 'WindDirec (deg)' ]
						: null;
				processedData.windDirection =
					unprocessedData[ 'WindDirec (deg)' ] !== '-9999.00'
						? getSimpleDirection(
								unprocessedData[ 'WindDirec (deg)' ]
						  )
				...
				processedData.windSpeed =
					unprocessedData[ 'WindSpeed (m/s)' ] !== '-9999.00'
						? Math.round(
								parseFloat(
									unprocessedData[ 'WindSpeed (m/s)' ]
								) * 1.94384
						  )
						: null;
				processedData.barometer =
					unprocessedData[ 'Pressure (hPa)' ] !== '-9999.00'
						? parseFloat( unprocessedData[ 'Pressure (hPa)' ] )
						: null;
			...
			if ( buoyDataPoints[ 2 ]?.data_points && processedData.barometer ) {
				const unprocessedData = JSON.parse(
					buoyDataPoints[ 2 ]?.data_points
				);
				if ( unprocessedData[ 'Pressure (hPa)' ] !== '-9999.00' ) {
					const barometerPrevious = parseFloat(
						processedData.barometer
					);
					if ( processedData.barometer <= barometerPrevious ) {
						processedData.barometerChange = -1;
					} else {
						processedData.barometerChange = 1;
					}
					// processedData.barometerChange =
			...
			buoyDataPointsClone
				.splice( 0, Math.ceil( ( zoom / 7 ) * maxItems ) )
				.forEach( ( buoy ) => {
					...
					dataTypes.forEach( ( type ) => {
						...
						if (
							...
							chartData[ key ].labels.push(
								parseInt( buoy.timestamp ) * 1000
							);
							// Has arrows
							if ( type.rotate ) {
								chartData[ key ].datasets[ 0 ].rotation.push(
									Math.floor(
										parseFloat( rawData[ type.rotate ] )
									)
								);
								// Colour arrows
								if (
									type.rotateColour &&
									Number.isInteger( type.rotateColourMin ) &&
									Number.isInteger( type.rotateColourMax )
								) {
									let bracket = 0;
									if (
										parseInt( rawData[ col ] ) >
										type.rotateColourMax
									) {
										bracket = 8;
									} else if (
										parseInt( rawData[ col ] ) <
										type.rotateColourMin
									) {
										bracket = 0;
									} else {
										bracket = Math.round(
									...
									chartData[
										key
									].datasets[ 0 ].pointStyle.push(
										arrowImages[ bracket > 8 ? 8 : bracket ]
									);
								} else {
									// No colour
									chartData[
		...
		if ( buoyTideData.length > 0 ) {
			...
			processedData.tideTime = dayjs(
				buoyTideData[ 0 ].timestamp
			).format( 'h:mma' );
	...
	useEffect( () => {
		...
		// Fetch Map details
			...
			axios.get( '/wp-json/wac/v1/map' ).then( ( response ) => {
				if ( response.status === 200 ) {
	...
	}, [buoys, buoyDataPoints, buoyTideData, maxItems, mapDetails, zoom] ); // buoys, buoyDataPoints, buoyTideData, maxItems, mapDetails, zoom
	...
	const updateBuoy = ( newBuoyId ) => {
		...
		axios
			.get(
				'/wp-admin/admin-ajax.php?action=waf_rest_list_buoy_datapoints',
				{
					params: {
						id: newBuoyId,
						range: 10,
		...
		axios
			.get(
				'/wp-admin/admin-ajax.php?action=waf_rest_list_buoy_tide_data&start=' +
					now,
				{
					params: {
						id: newBuoyId,
			...
			.then( ( response ) => {
				if ( response.status === 200 ) {
					...
					const tideData = [ ...response.data.data ].reverse();
					// Loop through until greater than now
					for ( let i = 0; i < tideData.length; i++ ) {
						if (
							parseInt( tideData[ i ].timestamp ) > mapDetails.now
						) {
							setBuoyNextTide( {
								...
								isFalling:
									i > 0
										? tideData[ i ] < tideData[ i - 1 ]
										: true, // Can't know for sure if it's the first
							} );
							break;
						}
					}
	...
	const scrollSectionIntoView = ( name ) => {
		document.querySelector( name )?.scrollIntoView( {
			behavior: 'smooth',
		} );
	};

	return buoys.length > 0 ? (
		<>
						...
						>
							<Map
								...
								bounds={ {
									...
									north: parseFloat(
										mapDetails.maps_lat_min
									),
									west: parseFloat( mapDetails.maps_lng_min ),
									south: parseFloat(
										mapDetails.maps_lat_max
									),
								} }
								markers={ buoys
									.filter(
										( buoy ) => buoy.is_enabled === '1'
									)
									.map( ( buoy ) => ( {
										id: buoy.id,
										position: {
						...
						<>
							...
							<div className="observations">
								...
								<div className="observation-small sea-state">
									...
									{ seaState !== null ? (
										...
										>
											{ swellRating( seaState ) }
										</p>
									) : (
										<p>-</p>
									) }
								</div>
								<div className="observation-small sea-temperature">
									<button
										className="scroll-into-view"
										onClick={ () => {
											scrollSectionIntoView(
								...
								<div className="observation-small sea-tide">
									<button
										className="scroll-into-view"
										onClick={ () => {
											scrollSectionIntoView(
									...
									{ buoyNextTide ? (
										<p>
											<small className="tide-direction">
												{ buoyNextTide.isFalling ? (
													<>
														Falling{ ' ' }
														<IconArrowDown />
													</>
								...
								<div className="observation-small sea-barometer">
									<button
										className="scroll-into-view"
										onClick={ () => {
											scrollSectionIntoView(
												'.chart-wrapper.barometer'
											);
										} }
									>
										Scroll into view
							...
							<div className="historic-observations">
								<div className="chart-wrapper wind">
									...
									<LineChart
										data={
											selectedBuoy.chartData.windSpeed
										}
								...
								<div className="chart-wrapper sea-state">
									...
									<LineChart
										data={
											selectedBuoy.chartData.seasHeight
										}
										heading="Seas (m)"
										smooth={ 0 }
									/>
								...
								<div className="chart-wrapper barometer">
									...
									<LineChart
										data={
											selectedBuoy.chartData.barometer
										}
